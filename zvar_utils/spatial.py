...
def get_field_id(ra, dec):
    ...
    fieldno, ra_all, dec_all = (
        ZTFFieldData.fieldno,
        ZTFFieldData.ra_all,
        ZTFFieldData.dec_all,
    )

    deg = np.pi / 180.0
    ra = ra * deg  # convert to radians
    dec = dec * deg  # convert to radians
    ra_all = ra_all * deg  # convert to radians
    dec_all = dec_all * deg  # convert to radians
    ...
    ADIST_MAX = (
        5.66  # maximal angular distance between the center of the field and its edge
    )

    for i in range(nf):
        adist = ang_dist(ra, dec, ra_all[i], dec_all[i])
        if adist >= ADIST_MAX:
            continue
        x, y = ortographic_projection(ra, dec, ra_all[i], dec_all[i])
