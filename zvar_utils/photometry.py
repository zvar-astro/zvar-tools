...
def process_curve(
    ra: float, dec: float, times: list, mags: list, magerrs: list
    ...
    skycoord = coord.SkyCoord(ra, dec, unit=(u.deg, u.deg), frame="icrs")
    ...
    barycorr_times -= np.min(
        barycorr_times
    )  # Subtract off the zero point (ephemeris may be added later)
...
def remove_deep_drilling(
    barycorr_times: np.ndarray, flux: np.ndarray, ferrs: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    dt = 40.0  # seconds
