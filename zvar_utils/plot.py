...
def plot_hr_diagram(candidate_list: List[VariabilityCandidate], band: str, figsize: tuple = (9, 8), output_path: str = None, show_plot: bool = False):
    ...
    sample_path = os.path.join(os.path.dirname(__file__), '../data/hrd_query_edr3_200pc.fits')
...
def plot_lightcurve(candidate: VariabilityCandidate, time, flux, fluxerr, figsize: tuple = (9, 8), output_path: str = None, show_plot: bool = False):
...
def plot_periodicity(candidate: VariabilityCandidate, time: np.ndarray, flux: np.ndarray, fluxerr: np.ndarray, pgram: np.ndarray, best_period: float, show_plot: bool = True):
    ...
    fig.suptitle(f"ID: {candidate.id}, RA: {candidate.ra}, DEC: {candidate.dec}\nBest M: {candidate.best_M}", fontsize=16)
