...
def candidates_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--bands', type=str, default='g,r', help='Bands to process, separated by commas')
    parser.add_argument('--radius', type=int, default=5, help='Radius in arcseconds to search for xmatches in external catalogs')
    parser.add_argument('--periods_path', default='./data/periods', type=str, help='Path where to load the ZVAR files (e.g. periods for a field)')
    parser.add_argument('--output_path', type=str, help='Output directory for the CSV files')
    parser.add_argument('--credentials', type=str, help='Path to the JSON file with the Kowalski credentials')
