...
class VariabilityCandidate:
    def __init__(self, psid, ra, dec, valid, freq, fap, best_M, gaia_G=None, gaia_BP=None, gaia_RP=None, gaia_parallax=None, gaia_parallax_error=None, gaia_pmra=None, gaia_pmra_error=None, gaia_pmdec=None, gaia_pmdec_error=None, gaia_MG=None, gaia_BP_RP=None):
        self.id = psid
        self.ra = ra
        self.dec = dec
        self.valid = valid
...
def get_candidates(psids: np.ndarray, ra: np.ndarray, dec: np.ndarray, ratio_valid: np.ndarray, freqs: np.ndarray, sigs: np.ndarray) -> List[VariabilityCandidate]:
...
def add_gaia_xmatch_to_candidates(k: Kowalski, candidate_list: List[VariabilityCandidate], radius: float) -> List[VariabilityCandidate]:
    if not isinstance(candidate_list, list):
        raise ValueError('Candidates must be provided as a list')
    if not all(isinstance(candidate, VariabilityCandidate) for candidate in candidate_list):
...
def save_candidates_to_csv(candidate_list: List[VariabilityCandidate], field: int, band: int, path: str):
    if not isinstance(candidate_list, list):
        raise ValueError('Candidates must be provided as a list')
    if not all(isinstance(candidate, VariabilityCandidate) for candidate in candidate_list):
        raise ValueError('Candidates must be of type Candidate')
